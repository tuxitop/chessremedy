...
Feature 011 **consumes verified candidates only**. Feature 010's two-stage
detection pipeline (ADR-026) already owns:
...
A puzzle may originate only from a Feature-010 **verified candidate**: a
position where a tactic was available **on the user's turn** and the user did
not play the engine's best move (the user's own missed tactic / punished
...
A blunder or mistake that Feature-010 did **not** verify as a tactical
opportunity never becomes a puzzle. The spec records this direction against the
stale "opponent punishes the blunder" framing in
`research/puzzle-generation.md` §1 (see "Side to move" below): the 
position-centric, user-side model of ADR-026 v2 is authoritative.
...
Assembly is a pure, deterministic function of one verified candidate plus the
generator version:

```text
verified candidate → (candidate fields) + difficulty + puzzleGeneratorVersion
                      + solution/alternative data from the candidate
                      → immutable puzzle row
```

The assembled puzzle row contains:
...
user's move, so the user is the mover). The user trains their own decision
...
- **One puzzle per game position**, keyed `[sourceGameId + sourcePly]`.
  ...
  verified candidate to arrive for that game position (from whichever
  analysis) creates the row; any later candidate at the same `(game, sourcePly)`
  — including one from a re-analysis — maps to the existing row and is skipped
  (see re-analysis below).
...
- `inProgress` — the pass is running; a persisted progress counter
  (`done`/`total` over the analysis's verified candidates) advances as each
  candidate is assembled/written or skipped.
- `completed` — every verified candidate of the analysis was processed. A pass
  over zero verified candidates completes normally and is a real
  "no puzzles generated by this analysis".
...
- **Resumable on demand:** an interrupted, failed, or never-run pass for a
  ...
  "Resume / Run tactics scan". It processes only that analysis's verified
  candidates that do not yet have a puzzle for the game.
- **Cancellable:** a live pass can be cancelled in place; it stops at the next
  candidate boundary and leaves the state resumable (`queued`). Progress is
...
Each puzzle entry shows:

- a read-only Chessground board of the starting position (shared board
  wrapper; no engine, no interaction that mutates game/analysis data);
- difficulty (ADR-025 bucket and/or score);
- the tactical objective;
- provenance (source ply, the user's original move vs. the expected solution,
  generator/detection versions where useful).
...
- **Versions:** every puzzle stores `puzzleGeneratorVersion`, `detectionVersion`
  and `candidateGenerationVersion` (plus the candidate's engine/analysis
  metadata) so existing records remain identifiable after algorithm or engine
  changes (ARCHITECTURE §9, ADR-020).
...
- **Zero verified candidates** → the generation pass runs and completes with
...
- **Same position reached twice in one game at different plies** → two
  candidates → two rows (`sourcePly` differs). Transposition-level merging is
  out of V1 scope (documented limitation of per-candidate keying).
...
- Per-game generation work is bounded by the analysis's verified-candidate
  count (≤ Feature-010's `MAX_CANDIDATES_PER_GAME` cap of 16), so one pass is
  at most ~16 pure assembly + write steps.
...
- Progress persistence is one small IndexedDB write per settled candidate,
...
row/count and per-game puzzle-view tests.
...
- **Unit/domain:** pure assembly from deterministic verified-candidate
  ...
  zero-candidate completion, absent-vs-zero derivation.
...
- **Component:** Library row `puzzleCount` rendering across states (absent,
  ...
  engine, empty/state-note states, mobile layout, keyboard/AT behaviour.
- **End-to-end (committed engine fixture proof, mirroring Feature-010):** a
  ...
  Library/read surfaces) with a real engine and a real verified candidate.
...
- branching opponent defences beyond the verified forcing line;
