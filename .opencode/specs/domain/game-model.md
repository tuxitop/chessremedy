...
days-per-turn, estimated length, display string, profile) is parsed from the
raw string and persisted with the game (`specs/domain/time-control.md`). The
category is computed with the **platform profile** selected from the game's
`source` (`chesscom` / `lichess` / `generic` for `local`/`fixture`), so
`normalizedTimeControl` follows each platform's published definition; the
platform's own label is kept as an optional hint. Consumers display
`TimeControl.display` (house style `M|I`, e.g. `5|5`, `10|0`) and never
render raw seconds as minutes.
